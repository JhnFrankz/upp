...
2. **Delivery**: COMPLETED — 6 stacked PRs (#17–#22) MERGED to main, tip `77c385a` (merge of #22, 2026-08-12). Issue #16 (enhancement) tracks the change.
   ...
   - PRs #21/#22 = U5 command / U6 hint commits rebased onto updated main: `dd2a700`, `a234c35`. **Correction (2026-08-12, native review finding R3-tree-identity-false): the trees are NOT identical** to the settled candidates `ba0a6df` / `41edbad` (the verify-phase HEAD) — each pair differs by the lint-fix commits `950d5e5` / `e734103` (3 files, 10 insertions / 10 deletions). Verified: tree(ba0a6df)=`c6d09a70…` vs tree(dd2a700)=`b1fbee74…`; tree(41edbad)=`4a80dbf9…` vs tree(a234c35)=`1ef97252…` (equal to merged-main tree(77c385a)). Consequence: the verify evidence revision `bb882df1…` was computed on tree `4a80dbf9…`; the delivered tree `1ef97252…` is covered by the post-delivery full-suite re-run on merged main (item 3 below).
