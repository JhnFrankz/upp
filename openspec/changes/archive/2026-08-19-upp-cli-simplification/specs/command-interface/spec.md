...
**Reason**: `upp export` and `upp import` subcommands are removed from the CLI. Configuration backup, sharing, and version control are performed using standard filesystem commands (`cp`, `cat`) and dotfiles management on `~/.config/upp/config.toml` (or `%APPDATA%/upp/config.toml`), removing redundant CLI surface area.
...
(Previously: `--dry-run` had no single-letter shorthand `-n`.)
