...
- [x] 3.1 [RED-pin][S] T3: `internal/cli/update_test.go` NEW `TestRunUpdate_DefaultGroupSummarySkipsDeselected` (~35 lines): apt owns gh+docker; gh updates, docker prompt-denied → flat summary "Updated: gh" AND "Skipped: docker". Expected GREEN on HEAD (D4: no production change); RED ⇒ STOP and escalate (out-of-scope default-path bug). Pins ux-patterns "Default group bulk summary". (Executed GREEN on HEAD as designed — 36 lines, commit b6d1a54. Flat `detailSummary` prints "Updated:"/"Skipped:"; second prompt hits stdin EOF → ConfirmDeny → StatusSkipped.)
- [x] 3.2 [M] `internal/output/render_test.go` [T1] DELETE 8 tests + section comments (:946-954, :1033-1041): `TestGroupBulkSummary_UpdatedAndSkipped` :959; `_PartialFail` :987; `_DryRun` :1011; `TestGroupBatchPreview_RendersPlannedBatch` :1046; `_CurrentAndUngated` :1077; `_CheckFailed` :1101; `TestGroupBulkSummary_CanonicalOrderNotStatusOrder` :1124; `_ExplicitCountsAndOrdering` :1177. SURVIVE: all `UpdateSummary` tests (incl. :1149, :1208). (All 8 deleted by name, −234 lines; both UpdateSummary pins survive — post-deletion :946 verbose diagnostics, :971 no-misleading-AllClean.)
- [x] 3.3 [M] `internal/output/render.go` [D3]: delete `GroupBulkSummary` type+method (:376-467), `GroupBatchTool` (:471-485), `GroupBatchPreview` type+method (:489-~540). Flat `UpdateSummary` (:322/:406/:467) untouched. (Deleted whole region :374-533 incl. WU5 section comment, −161 lines; zero remaining references verified by grep; helpers statusLabel/statusIcon shared with surviving ListTools path retained. Commit 1ccbfc5.)
- [x] 3.4 [S] FINAL VERIFICATION: `go test ./... -count=1`; `go vet ./...`; `gofmt -s -l` empty; `bash scripts/smoke-test.sh --skip-build` green; flag surface exactly 5 (`--quiet/-q`, `--verbose/-v`, `--ci`, `--only` global; `--dry-run/-n` update+uninstall); `--manager`/`--update-group`/`--skip` all rejected non-zero. (All green: 10 pkgs ok; vet clean; gofmt empty; `make build` ok; smoke 33/33; surface = --ci/--only/--quiet/-q/--verbose/-v global + -n,--dry-run update + --dry-run uninstall [no -n shorthand on uninstall — pre-existing, untouched since main]; --manager/--update-group/--skip exit 1 at root and update subcommand.)
