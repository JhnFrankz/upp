...
(Previously: the standalone tools section contained only runtimes and tools such as nvm, npm, pnpm, bun, opencode, and go-on-Linux; `uv` was not rendered on the board.)
...
(Previously: `uv` was not part of the standalone tools included in the summary report.)
