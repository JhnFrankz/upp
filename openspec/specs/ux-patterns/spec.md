...
(Previously: manager-group summaries were only generated when explicitly triggered via `--manager`/`--update-group` opt-in flags; default runs did not render manager-group package updates or per-tool group outcomes. The summary is a single flat report — no dedicated group renderer exists. Earlier: deterministic summary ordering was handled by CLI-internal `runChecks` slotting; deterministic outcome ordering is now guaranteed by `engine.Check` index slotting and mapped to presentation models.)
...
The brew manager row MUST render as current in interactive TTY `upp update` pre-check board runs and in `upp list`, because the brew adapter reports no self-update availability signal by design (`check()` sets `Latest=Current`, `UpdateAvailable=false`). The pending set — the interactive CheckboxSelector and the `upp update --dry-run` (`-n`) report — MUST instead be derived from `engine.Plan` (`plan.Updates`), which plans `PolicyAlwaysUpdate` tools unconditionally (`internal/engine/plan.go:92-97,153-164`); therefore `PolicyAlwaysUpdate` managers and standalone tools (e.g. brew, bun) MUST appear in the TTY pending selector AND MUST report as planned actions in `-n`, even when currently up to date. Rendering them as current in `-n` would claim nothing would change while a real run executes their update command. apt, winget, and scoop rows MUST render planned self-update actions in `-n` when their `check()` reports availability.
