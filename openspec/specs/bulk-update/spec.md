...
Define the system's ability to update a manager's owned tools as one group (bulk update), executed by default during standard `upp update`. This restores bulk-upgrade behavior anchored PER-MANAGER (not per-arbitrary-command), using the ownership model from vision point 6, so owned tools (gh, docker, go) are updated via their manager's package command (`UpdatePackage`) by default alongside standalone tools.
...
(Previously: the requirement stated `--manager <mgr>` and `--update-group <mgr>` MUST remain supported as explicit filters restricting execution to the specified manager group; the opt-in trigger path is deleted and the default delegated path is the only update path. Earlier still: `Opt-In Group Bulk Trigger` specified that bare `upp update` must not trigger group bulk updates.)
...
(Previously: the requirement excluded owned tools named by `--skip <owned-tool>` and enumerated only on explicit `--manager` invocation; `--skip` is removed and batch narrowing is expressed through the existing `--only` filter.)
