...
The system MUST provide sensible defaults for all settings. Defaults MUST be applied when fields are absent from the config file.

| Scenario | GIVEN | WHEN | THEN |
|----------|-------|------|------|
| Empty config | Config file is empty/missing | Config loaded | All defaults applied |
| Partial config | Only `[settings]` section present | Config loaded | Tool sections default to platform catalog |
